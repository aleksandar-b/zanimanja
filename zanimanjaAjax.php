...
class Ajax extends CI_Controller {
	...
	public function videos() {
		...
		if ($data['info'] = $this->Grafikon_model->get_videos($zanimanje)) {
			echo '<div class="center"><h4><strong><a href="'.base_url().'users/videos/">VIDEOS</a></strong></h4></div>';

			foreach ($data['info'] as $info1) {

				echo '<div class="display"><div class="time">'.$info1->time.'</div><div class="tution">'.$info1->tution.'<div class="hidden">'.$info1->description.'</div></div><div class="red"><a target="_blank" href="'.$info1->videos_url.'">'.$info1->name.'</a></div></div></br>';
	...
	public function mentors() {
		...
		if ($data['info'] = $this->Grafikon_model->get_mentors($zanimanje)) {
			echo '<div class="center"><h4><strong><a href="'.base_url().'mentor/all/">MENTORS</a></strong></h4>></div>';
	...
	public function schools() {
		...
		if ($data['info'] = $this->Grafikon_model->get_schools($zanimanje)) {
			echo '<div class="center"><h4><strong><a href="'.base_url().'schools/all/">OBRAZOVNE USTANOVE</a></strong></h4></div>';

			foreach ($data['info'] as $info1) {

				echo '<div class="display"><div class="time">'.$info1->vreme.'</div><div class="tution">'.$info1->novac.'<div class="hidden">'.$info1->opis.'</div></div><div class="red"><a target="_blank" href="'.$info1->link.'">'.$info1->naslov.'</a></div></div></br>';
